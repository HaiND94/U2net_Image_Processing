        ...
        for j in range(im_array.shape[1]):
            if im_array[i][j] == 0:
                ...
                if j > max_j:
                    ...
                    min_j = j
    min_i = 0 if (min_i - OFFSET_EDGE) < 0 else min_i - OFFSET_EDGE
    min_j = 0 if (min_j - OFFSET_EDGE) < 0 else min_j - OFFSET_EDGE
    max_j = W if (max_j + OFFSET_EDGE) > W else max_j + OFFSET_EDGE
    max_i = H if (max_i + OFFSET_EDGE) > H else max_i + OFFSET_EDGE
...
def _detect_ob_region(im_frame, im_ob_tmp, debug=False):
    ...
    if debug:
        gray.show()
    ob_region = gray.point(lambda x: 0 if x > 160 else 255, '1')
    mask = gray.point(lambda x: 0 if x > 20 else 255, '1')
    if debug:
        ob_region.show()
        mask.show()
...
def _detect_text_region(im_frame, im_text_tmp, debug=False):
    ...
    gray = new_im.convert("L")
    mask = gray.point(lambda x: 0 if x < 100 else 255, '1')
...
def _change_location(im, coordinate, size_img, show_orig=False):
    ...
    if width_ob != width_region or height_ob != height_region:
        div = max(width_ob/width_region, height_ob/height_region)
...
def top_songs(net, list_img_origin, frame_template,
              list_img_ob, img_txt,
              path_out,
              collum_mode=1,
              js_txt='', debug=False):
    ...
    for index, img_ori in enumerate(list_img_origin):
        ...
        img_ori_tmp = BytesIO()
        _im_ori = crop_img(im_ori, debug=debug)
        _im_ori.save(img_ori_tmp, format="png")
        old, im, _pred, _pred_blur = get_object(img_ori_tmp, net, show_img=debug)

        _size_old = old.size

        if (_size_old[0] < _size_ob_tp_region[0] / 1.6) or (_size_old[1] < _size_ob_tp_region[1] / 1.6):
        ...
        mask = mask.convert("L")
        im_frame = Image.composite(im_frame, _im_frame_tmp, mask=mask)
...
if __name__ == "__main__":
    ...
    #
    # list_img_ob = ["./develop_env/top_songs/ex3/ob.png"]
    # img_txt = "./develop_env/top_songs/ex3/text.png"
    # frame_template = "./develop_env/top_songs/ex3/frame.png"

    list_img_ob = ["./develop_env/template_render/ex8/ob1.png"]
    img_txt = "./develop_env/template_render/ex8/text.png"
    frame_template = "./develop_env/template_render/ex8/frame.png"
    ...
    result = top_songs(net, list_img_origin, frame_template,

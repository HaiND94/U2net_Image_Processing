...
def get_object(img, net, show_img=False, debug=False, auto=False, top_song=False):
    ...
    new_img = new.crop(box)
    blur_img = _new_blur.crop(box)
    pred = pred.crop(box)
    pred_blur = pred_blur.crop(box)
    ...
    del im, empty_img, new, _new_blur, _blur_mask
    if auto is True and top_song is False:
        return ob_position, new_img, blur_img, pred, pred_blur
    elif top_song:
        return ob_position, top_status, new_img, blur_img, pred, pred_blur
    else:
        return new_img, blur_img, pred, pred_blur
...
if __name__ == "__main__":
    ...
    img = '../../../develop_env/images/error.jpg'
    # print(img.size)
    path = '../../../model/u2net.pth'
    net = load_model(path, 'u2net')
    get_object(img=img, net=net, show_img=True, debug=True)

...
def grayscale(net, path,
              path_out,
              collum_mode=1,
              js_txt='', debug=False):
    """
Detail : Made back ground to gray color
    :param path: image path
    :param net: model use for object detect to remove ob
    :param path_out: to save image
    :param js_txt: list text song
    :param collum_mode: the collum draw into image
    :param debug: use for debug, if you want to debug, may be you need set True
    ...
    text_region = _text_region["region"]
    ...
    if len(text_region) == 1:
        if len(text_region[0]) == 4:
            text_region = text_region[0]
    elif len(text_region) == 2:
        try:
    ...
    del im, pred, gray, new_img, template
...
if __name__ == '__main__':
    ...
    print(grayscale(net, path=path_ground,
